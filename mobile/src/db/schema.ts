...
SELECT r.id, r.logged_at, r.date_str, r.distance_mi, r.battery_used_pct,
    r.draw_rate, r.wh_used, r.range_remaining_mi, r.ride_mode, r.odometer_after, r.notes,
    CASE WHEN r.draw_rate <= 1.5 THEN 'A' WHEN r.draw_rate <= 2.5 THEN 'B'
         WHEN r.draw_rate <= 3.5 THEN 'C' ELSE 'D' END AS efficiency_grade
FROM ride_log r ORDER BY r.logged_at DESC;
CREATE VIEW IF NOT EXISTS v_ride_stats AS
SELECT COUNT(*) AS total_missions, ROUND(SUM(distance_mi),1) AS total_miles,
    ROUND(AVG(distance_mi),1) AS avg_miles_per_mission,
    ROUND(AVG(battery_used_pct),1) AS avg_battery_used_pct,
    ROUND(AVG(draw_rate),2) AS avg_draw_rate, ROUND(MIN(draw_rate),2) AS best_draw_rate,
    ROUND(MAX(draw_rate),2) AS worst_draw_rate, ROUND(SUM(wh_used),0) AS total_wh_used,
    MAX(logged_at) AS last_mission_at FROM ride_log;
CREATE VIEW IF NOT EXISTS v_battery_trend AS
SELECT DATE(logged_at) AS ride_date, ROUND(AVG(draw_rate),2) AS avg_draw_rate,
