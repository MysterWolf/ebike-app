      ...
      <View style={{ flexDirection: 'row', gap: 5, backgroundColor: 'transparent',
        ...
        {[d1, d2, d3].map((d, i) => (
          <Animated.View key={i} style={[{ width: 7, height: 7, borderRadius: 4,
            backgroundColor: dotColor }, { transform: [{ translateY: d }] }]} />
        ))}
      </View>
    </View>
  );
...
export function ChatPanel({ state, update, isTyping, onSend }: Props) {
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    apiLabel: { fontFamily: MONO, fontSize: 9, color: C.inkMid, letterSpacing: 0.5 },
    apiInput: {
      flex: 1,
    ...
    msgWrap:     { maxWidth: '85%', alignSelf: 'flex-start', gap: 3 },
    msgWrapUser: { alignSelf: 'flex-end', alignItems: 'flex-end' },
    bubble:          { borderRadius: 10, paddingHorizontal: 12, paddingVertical: 9 },
    bubbleUser:      { backgroundColor: C.accentTint, borderWidth: 1, borderColor: C.accentTint },
    bubbleAssistant: { backgroundColor: C.white, borderWidth: 1, borderColor: C.border },
    bubbleSystem:    { backgroundColor: C.accentTint, borderWidth: 1, borderColor: C.accentTint },
    bubbleText:  { fontSize: 13, lineHeight: 19, color: C.ink },
    systemText:  { fontFamily: MONO, fontSize: 11, color: C.accent },
    msgTime:     { fontFamily: MONO, fontSize: 9, color: C.muted, marginLeft: 4 },
    ...
    quickScroll:  { backgroundColor: C.white, borderTopWidth: 1, borderTopColor: C.border, maxHeight: 44 },
    quickContent: { paddingHorizontal: 10, paddingVertical: 8, gap: 6, flexDirection: 'row' },
    quickBtn:     {
      backgroundColor: C.background, borderWidth: 1, borderColor: C.border,
      borderRadius: 14, paddingHorizontal: 12, paddingVertical: 5,
    },
    quickLabel: { fontFamily: MONO, fontSize: 11, color: C.inkMid },
    inputBar: {
    ...
    chatInput: {
      ...
      fontSize: 14, color: C.ink, lineHeight: 20, minHeight: 40, maxHeight: 80,
    },
    sendBtn:         { backgroundColor: C.accent, borderRadius: 8, paddingHorizontal: 14, paddingVertical: 10, alignSelf: 'stretch', justifyContent: 'center' },
    sendBtnDisabled: { opacity: 0.35 },
    sendLabel:       { fontFamily: MONO, fontSize: 11, fontWeight: '700', letterSpacing: 1, color: '#FFFFFF' },
  }), [C]);
  ...
  return (
    <KeyboardAvoidingView style={styles.container}
      ...
      <View style={styles.apiRow}>
        ...
        <TextInput style={styles.apiInput} value={state.apiKey}
          onChangeText={v => update({ apiKey: v })}
          placeholder="sk-ant-api03-..." placeholderTextColor={C.muted}
      ...
      <ScrollView ref={scrollRef} style={styles.messages} contentContainerStyle={styles.messagesContent}
            ...
            <View key={i} style={[styles.msgWrap, isUser && styles.msgWrapUser]}>
              <View style={[
                ...
                {isSystem ? (
                  ...
                  <Text style={styles.bubbleText}>{renderMarkdown(msg.content, C.accent)}</Text>
                )}
              </View>
        ...
        {isTyping && <TypingIndicator dotColor={C.inkMid} />}
      </ScrollView>

      <ScrollView horizontal showsHorizontalScrollIndicator={false}
        style={styles.quickScroll} contentContainerStyle={styles.quickContent}>
        {Object.entries(QUICK_QUERIES).map(([key, query]) => (
          <TouchableOpacity key={key} style={styles.quickBtn}
            onPress={() => onSend(query)} activeOpacity={0.7}>
      ...
      <View style={styles.inputBar}>
        <TextInput style={styles.chatInput} value={input} onChangeText={setInput}
          placeholder="Ask the analyst..." placeholderTextColor={C.muted}
          multiline returnKeyType="send" onSubmitEditing={send} blurOnSubmit={false} />
        <TouchableOpacity style={[styles.sendBtn, (!input.trim() || isTyping) && styles.sendBtnDisabled]}
          onPress={send} disabled={!input.trim() || isTyping} activeOpacity={0.8}>
